...
## 13. V4 Main昇格PR時点の位置づけ

Step 12-B の V4 Main昇格PRでは、root `index.html` を V4 相当のMainシェルに更新します。

```text
- root index.html: Main本番URLで表示される V4 相当シェル。
- root index.html のCSS参照: ./v4/css/app.css
- root index.html のJS参照: ./v4/js/main.js
- v3/: V3.1 過去版資産として維持。
- v4/: V4 実行資産として維持し、/v4/ 確認URLでも引き続き利用。
- archive/index_v3_1_main_before_v4.html: V4 Main昇格前の root index.html 退避版。
```

Main昇格PRでは、Firestore 実データ、Firebase / Firestore 設定、`users_v2`、`country_masters`、`masters/gen_{1..9}` は変更しません。
