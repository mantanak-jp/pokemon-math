...
| `canary/index.html` | V2図鑑遷移プロトタイプ。 | 参考用です。V2本実装の正本にはしません。 |
...
`canary/` 配下には、現時点で少なくとも以下の3つのHTMLがあります。
...
- 世代選択時に仮グリッドを描画します。
- 詳細モーダルの概念だけを持ちます。
- Firestore連携や実データ連携は本実装レベルではありません。
...
- 新規ファイル名は、実装前にIssueまたは仕様書で明確にします。
- 既存 `canary/index.html`、`canary/index_ui.html`、`canary/index_dev_masterdata_2.html` は参照・移植元としてのみ扱います。
