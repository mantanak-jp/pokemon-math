...
V2 を Main に昇格する前に、Canary 段階で運用者が確認・実行する事前移行処理として扱います。
...
したがって、移行ツールから `users_v2/guest` へは通常書き込みません。

ただし、Canary検証時に限り、世代クリア直前状態を作るために `users_v2/guest` を検証用データへ上書きする補助版を一時的に使用することがあります。これは移行仕様ではなく、Canary検証用の一時操作です。
...
Main昇格時も、`canary/migration.html` は通常導線に混ぜません。

Main昇格後の V2 本体は `users_v2` を正本として読みます。
...
- createdAt / updatedAt / migratedAt / migrated_from_v1 を保存しない。
```
...
Step 6-8 時点で、V1.5→V2移行は以下の状態です。
