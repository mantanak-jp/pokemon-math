...
`v3/index.html` のメニュー画面にレベルボタンを表示する。
...
`v3/js/app.js` では、`[data-level]` を持つボタンに一括でイベント登録している。
...
`v3/js/quiz.js` では、`makeQuestionForLevel(level)` に問題生成が集約されている。

V3.1では、レベル4・5・6を明示分岐化する。
...
単純なランダム再試行でも実装できるが、V3.1では条件を満たす候補を先に列挙し、その中からランダムに1問を選ぶ。

この方式により、`63 ÷ 3 = 21` や `72 ÷ 6 = 12` のような問題が出題対象に入る。
...
レベル5・6はいずれも数値回答であり、既存の数値4択生成で対応可能と判断する。

ただし、正解が1の場合、候補は0以上に制限されるため、レベル5では `0` が選択肢に入る可能性がある。初期実装では許容する。
...
V3.1の算数レベル追加は小さめの差分で済むため、実装PRは基本的に1本でよい。
