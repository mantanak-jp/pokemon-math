...
- Codex app のローカル作業範囲と実行環境を明確にする
...
現在の基本運用は `ChatGPT + iPhone経由Codex app + ローカルPC上の実行環境 + GitHub` です。
...
- 差分確認
- 検証補助
- ローカルPC上の `C:\Users\manta\dev\pokemon-math` を実行環境として使用
...
User:
- iPhone の ChatGPT アプリ内 Codex リンクから指示
- 最終判断
- merge 判断
- 実機確認
- 実データに関わる判断
...
- Codex app はローカルPC上のリポジトリで実行する
- ユーザーがローカルPCを直接操作する前提にしない
...
2026-06-11 の Step 1.5 作業では、Codex連携運用を反映するための最小更新のみ許可されています。
...
V4 開発では、確認対象は原則 `/v4/` ディレクトリです。GitHub Pages の `/v4/` を iPhone で確認する作業は、通常は対象 PR が merge された後に行います。
