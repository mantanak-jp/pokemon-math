...
Step 8-A では、同ツールに Firestore `country_masters` への投入機能を追加します。投入先は `country_masters` collection、document id は `record.country_id` です。既存 document がある場合は同じ document id に上書きします。

実投入は Step 8-B の明示承認後にのみ行います。Step 8-A のPRでは Firestore 実データへの作成・更新・削除は行いません。
...
- Firestore実投入
- Firestore実データ操作
- country_masters collection の実作成
- data/country_masters.generated.json の内容変更
- V4実装
- 国旗クイズ実装
```
...
```text
- ページロードだけで Firestore に書き込まないこと
- 投入ボタンが初期状態で disabled であること
- JSON parse、validation、件数、enabled=false の条件を満たすまで投入ボタンが有効にならないこと
- 確認チェックボックスと確認テキスト IMPORT country_masters の完全一致が必要であること
- クリック時に window.confirm で最終確認を出すこと
- 投入先 collection が country_masters であること
- document id が record.country_id であること
- data/country_masters.generated.json を変更していないこと
- v4/ を変更していないこと
```
