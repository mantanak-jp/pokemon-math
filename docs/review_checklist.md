...
V4国旗クイズ実装PRでは、以下も確認します。

```text
- 期待バージョン表示がPR本文に明記されている
- Step 9-A の期待表示は V4開発版 v4.0.0.1.202606120001
- root index.html / v3/ を変更していない
- Firestore country_masters は読み取り専用である
- Firestore 実データ、users_v2、masters/gen_{1..9} を変更していない
- さんすうクイズの既存導線が維持されている
- こっきクイズで5問4択が生成される
- 不正解時に正解選択肢が分かる
- Step 9-A では報酬接続を完了扱いにしていない
```

V4国旗クイズ報酬接続PRでは、以下も確認します。

```text
- 期待バージョン表示がPR本文に明記されている
- Step 9-B の期待表示は V4開発版 v4.0.0.2.202606120002
- root index.html / v3/ を変更していない
- data/country_masters.generated.json / tools/ を変更していない
- 算数クイズの報酬条件を変更していない
- 国旗クイズのみ 2〜3問正解で1匹、4〜5問正解で2匹になっている
- country_masters は読み取り専用で、書き込みを追加していない
- users_v2 の構造変更をしていない
- クイズ選択ボタンに報酬条件が表示されている
- 国旗クイズ問題文が iPhone で大きすぎないサイズに調整されている
```

V4国旗クイズ報酬条件微修正PRでは、以下も確認します。

```text
- 期待バージョン表示がPR本文に明記されている
- Step 9-C の期待表示は V4開発版 v4.0.0.3.202606120003
- 算数クイズの報酬条件を変更していない
- 国旗クイズのみ 0〜2問正解で0匹、3〜4問正解で1匹、5問正解で2匹になっている
- こっきクイズの説明が「3もんで1ぴき・5もんで2ひきゲット」になっている
- country_masters / users_v2 / masters/gen_{1..9} を変更していない
- root index.html / v3/ を変更していない
```
...
## V4 Phase 4完了報告PRチェック

V4 Phase 4完了報告PRでは、以下を確認します。

```text
- docs/v4_phase4_completion_report.md が追加されていること
- documentation-only changes であること
- runtime files を変更していないこと
- root index.html / v3/ / v4/ を変更していないこと
- canary/ / archive/ を変更していないこと
...
- Firestore 実データを変更していないこと
- 期待バージョン表示が V4 v4.0.0.3.202606120003 であること
